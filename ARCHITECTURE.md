...
| **Impostazioni** | ✅ Organizzata in sezioni (non più un unico elenco di card): **Famiglia & Aspetto** (nome famiglia mostrato nel saluto in Home; palette di sfondo dell'app, si applica subito), **Meteo** (nome da visualizzare, latitudine/longitudine — priorità sulle coordinate, niente geocoding se compilate), **Menu scuola** (template rotazione 4 settimane, merende, punto di partenza ciclo — invariato), **Integrazioni** (credenziali Google Calendar/Bring!/Garmin: se compilate qui vincono su `.env`, per cambiarle senza accedere al NUC; i campi segreto non vengono mai rimandati al frontend dopo il salvataggio, solo un flag "impostato") | `homehub.app_config` (key/value) via `GET/PUT /api/settings` — vedi `backend/app/core/runtime_settings.py` (unico lettore, con fallback a `.env` campo per campo) e `backend/app/api/routes/settings.py` (unico scrittore, invalida anche le sessioni già autenticate degli adapter e la cache del geocoding se le credenziali/il meteo cambiano) |
