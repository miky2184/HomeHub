...
| **Menu** | Menu scolastico a **rotazione di 4 settimane** (il volantino della scuola, invariato per mesi) + merende mattina/pomeriggio fisse per giorno + cena di casa | ✅ Template + ancora ciclo su Postgres (data entry in Impostazioni, non in Cucina/Home che sono sola lettura) + ✅ cena di casa letta direttamente da `dieta.menu_settimanale` (stesso Postgres, altra web app dell'utente — il piano nutrizionale copre tutta la famiglia), niente API REST di mezzo — vedi `backend/app/db/dieta_models.py:menu_settimanale_table` |
...
- **Fase 2 — Menu & Allenamenti**: ✅ fatto. Tab Menu con template scuola/merende a rotazione (data entry in Impostazioni) + ✅ cena di casa letta da `dieta.menu_settimanale`; tab Allenamenti da Garmin Connect + `dieta.allenamento`.
