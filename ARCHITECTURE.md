...
| **Home Inventory** | ✅ **Sola lettura**: oggetti scaduti o in scadenza entro 7gg (stessa soglia "warning" della web app dedicata), con contenitore associato. Gestione (creare/modificare/consumare/quantità) resta nella web app `home_inventory_web`. ✅ Un pulsante "Aggiungi a Bring!" per ogni alert manda l'articolo (nome + quantità/unità come specifica) sulla lista della spesa in un click. ✅ **"Sfoglia per contenitore"**: selettore con tutti i contenitori (anche vuoti) e il loro contenuto completo, non solo ciò che scade a breve — sostituisce il foglio di carta sul frigo per sapere cosa c'è nei cassetti del freezer (cucina/terrazzo), senza doverli aprire | Lettura diretta dello schema Postgres `home_inventory` (stesso Postgres di homehub, altra web app dell'utente — niente API REST, come per `dieta.allenamento`) — vedi `backend/app/db/home_inventory_models.py` e `get_inventory_by_container` in `backend/app/services/aggregator.py`. Scrittura verso Bring! riusa `/api/shopping` (stesso adapter della tab Spesa) |
