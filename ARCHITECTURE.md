...
| **Spedizioni** | ✅ Tracking manuale di pacchi: numero di tracking + corriere (per ora solo Poste Italiane ha aggiornamento automatico, "Altro corriere" è solo un promemoria senza refresh) + nota libera. Stato e storico eventi letti **on-demand** (nessuno scheduler in background, coerente col resto dell'app): al caricamento del tab/della card Home, le spedizioni Poste non consegnate il cui ultimo poll è più vecchio di 30' vengono ricontrollate in un'unica chiamata batch; pulsante "Aggiorna" per un refresh manuale immediato di una singola spedizione. In Home una card mostra il conteggio in transito + le prime, con link al dettaglio storico eventi in modale. ✅ Nel dettaglio, **mappa del percorso indicativo** (Leaflet + tile OpenStreetMap, marker su ogni città/centro di smistamento distinto dello storico, geocodificati on-demand solo all'apertura): Poste non fornisce indirizzi di mittente/destinatario nel tracciamento pubblico (solo nomi di luogo, per motivi di privacy — comune a tutti i corrieri), quindi è un percorso a livello di città/hub, non porta a porta | Tabella propria `homehub.shipment` — vedi `backend/app/db/models.py:Shipment`, `backend/app/api/routes/shipments.py` e `refresh_shipment`/`refresh_stale_shipments`/`get_shipment_summary`/`get_shipment_route` in `backend/app/services/aggregator.py`. Poste Italiane: endpoint non ufficiale usato dalla pagina "Dove Quando" del sito, vedi `backend/app/adapters/poste_italiane.py`. Geocoding luoghi: Nominatim/OpenStreetMap (gratuito, senza chiave), vedi `backend/app/adapters/geocoding.py` e `frontend/src/components/ShipmentRouteMap.tsx` |
...
| Spedizioni | CRUD completo: creare (numero + corriere + nota), modificare, eliminare (con conferma). Aggiornamento automatico se stantio (>30') al caricamento del tab/della Home + pulsante "Aggiorna" per un refresh manuale immediato di una singola spedizione |
...
- `shipment(id, tracking_number, carrier, label, status, delivered boolean, last_event_at, last_event_description, last_event_location, events jsonb, last_polled_at, last_poll_error, created_at, updated_at)` — tracking spedizioni (Spedizioni), stato/storico letti on-demand dal corriere (vedi §7.3), vedi migrazione `0006_shipments.py`
...
- **Poste Italiane**: nessuna libreria, chiamata diretta con `httpx` all'endpoint non ufficiale della pagina "Dove Quando" del sito (nessuna credenziale richiesta) — ✅ integrato (`backend/app/adapters/poste_italiane.py`).
- **Geocoding luoghi spedizione**: Nominatim (OpenStreetMap, gratuito e senza chiave) per risolvere i nomi di città/centro di smistamento in coordinate — ✅ integrato (`backend/app/adapters/geocoding.py`), mappa lato frontend con Leaflet + tile OpenStreetMap (`frontend/src/components/ShipmentRouteMap.tsx`).
...
- **Fase 3.2 — Spedizioni**: ✅ fatto. Tracking manuale di pacchi (per ora solo Poste Italiane con aggiornamento automatico on-demand, nessuno scheduler in background), CRUD completo nel tab dedicato, card riepilogo in Home — vedi `backend/app/api/routes/shipments.py`, `backend/app/adapters/poste_italiane.py` e `frontend/src/pages/ShipmentsPage.tsx`.
